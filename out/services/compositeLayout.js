...
exports.parseCompositeLeafInfos = parseCompositeLeafInfos;
exports.parseCompositeFieldInfos = parseCompositeFieldInfos;
exports.inferDataTypeFromTypeText = inferDataTypeFromTypeText;
function parseCompositeLeafInfos(ptypeOutput) {
...
function parseCompositeFieldInfos(ptypeOutput) {
    const lines = ptypeOutput.split(/\r?\n/);
    const headerIndex = findCompositeHeaderIndex(lines);
    if (headerIndex < 0) {
        return [];
...
function inferDataTypeFromTypeText(typeText, byteSize) {
...
function findCompositeHeaderIndex(lines) {
...
function parseCompositeFieldsBlock(lines, startLine) {
    const fields = [];
    let i = startLine;
    while (i < lines.length) {
        const statement = readLogicalStatement(lines, i);
...
function readLogicalStatement(lines, startLine) {
    const parts = [];
...
function isIgnorablePtypeLine(stripped) {
    return /^(public|private|protected)\s*:\s*$/i.test(stripped) || /total size \(bytes\)/i.test(stripped);
}
function normalizeStatementText(parts) {
    return parts.join(' ').replace(/\s+/g, ' ').trim();
}
function isCompositeStartText(text) {
    return /^(struct|class|union)\b[\s\S]*\{\s*$/i.test(text) || text === '{';
}
function parseCompositeClosing(text) {
...
function parseLeafInfos(text, statementByteSize) {
    if (!text.endsWith(';')) {
        return [];
    }
    if (/^(public|private|protected)\s*:\s*$/i.test(text)
...
function parseDirectFields(text, statementByteSize) {
    if (!text.endsWith(';')) {
        return [];
    }
    if (/^(public|private|protected)\s*:\s*$/i.test(text)
...
function parseInlineCompositeField(startText, statementByteSize, fieldName, arrayDims) {
...
function parseArrayDims(arraySuffix) {
    if (!arraySuffix) {
        return [];
    }
...
function expandArrayPrefixes(fieldName, dims) {
    if (dims.length === 0) {
        return [fieldName];
    }
...
function extractLayoutByteSize(line) {
...
function guessByteSize(typeText) {
 ...
 */
function isBaseClassSubobject(startText, closingFieldName) {
...
function extractInlineTypeName(startText) {
    const match = startText.match(/^(struct|class|union)\s+(.+?)\s*\{\s*$/i);
    return match?.[2]?.trim();
}
//# sourceMappingURL=compositeLayout.js.map