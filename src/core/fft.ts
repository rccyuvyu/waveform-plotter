export class FFT {
  private cachedWindowSize = 0;
  private cachedWindow: Float64Array = new Float64Array(0);
  private workN = 0;
  private workRe: Float64Array = new Float64Array(0);
  private workIm: Float64Array = new Float64Array(0);

  magnitudeSpectrum(input: number[]): number[] {
  ...
  fftSize(inputSize: number): number {
    return this.nextPowerOf2(inputSize);
  }

  private getWindow(size: number): Float64Array {
