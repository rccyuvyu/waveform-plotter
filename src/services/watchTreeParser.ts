export interface ParsedWatchNode {
  name: string;
  relativePath: string;
  expression: string;
  ...
  byteSize: number;
  address?: number;
  children: ParsedWatchNode[];
}

export interface ParsedWatchLeaf {
  fullName: string;
  expression: string;
  declaredTypeText: string;
  byteSize: number;
...
export function parsePtypeWatchTree(ptypeOutput: string, rootExpression: string): ParsedWatchNode | undefined {
  ...
  const rootNode: ParsedWatchNode = {
    name: rootExpression,
    relativePath: '',
...
export function flattenParsedWatchLeaves(rootName: string, root: ParsedWatchNode): ParsedWatchLeaf[] {
  const leaves: ParsedWatchLeaf[] = [];
  const walk = (node: ParsedWatchNode, currentPath: string) => {
...
export function flattenParsedWatchNodes(rootName: string, root: ParsedWatchNode): ParsedWatchNode[] {
  const nodes: ParsedWatchNode[] = [];
  const walk = (node: ParsedWatchNode, currentPath: string) => {
    nodes.push({
      ...node,
...
function parseFields(
  ...
  while (i < lines.length) {
    ...
    if (nestedMatch) {
      ...
      const isBaseClassSubobject = fieldName && nestedTypeName && (
        fieldName === nestedTypeName
...
function findNestedEnd(lines: string[], startIdx: number): { name: string; arraySize: number; endLine: number } {
  let depth = 1;
  let i = startIdx;
  while (i < lines.length) {
    const line = lines[i];
...
function composeMemberExpr(parentExpr: string, fieldName: string): string {
  return `(${parentExpr}).${fieldName}`;
}

function composeIndexExpr(parentExpr: string, index: number): string {
